...
impl PktReadOpts {
    ...
    pub fn is_stdin(&self) -> bool {
        self.pkts.is_none() && !self.no_pkts
    }
}

pub fn check_stdin(
    pkt_in: &PktReadOpts,
    read_in: &DataReadOpts,
    read_default_in: bool,
) -> anyhow::Result<()> {
    ensure!(
        !(pkt_in.is_stdin() && read_in.common.is_stdin(read_default_in)),
        "both pkts and read data is claiming stdin - change either (e.g. --no-pkts)"
    );
    Ok(())
}
...
static LK_MMAP_FILE_SIZE: std::sync::OnceLock<u64> = std::sync::OnceLock::new();
fn should_mmap(file_size: u64) -> bool {
    let min_mmap_size = *LK_MMAP_FILE_SIZE.get_or_init(|| {
        std::env::var("LK_MMAP_FILE_SIZE")
            .ok()
...
impl Reader {
    ...
    pub fn read_next_data(
        ...
        if self.delim.is_some() && self.delim == buf.last().copied() {
            buf.pop();
        }
        if self.eval {
    ...
    fn check_size(&mut self, buf: &mut Vec<u8>, freespace: usize) -> anyhow::Result<()> {
        ...
        if freespace < buf.len() {
            match self.on_overflow {
                EMode::Error => anyhow::bail!(
                    "read {} but packet only has {freespace} free space - ",
                    buf.len()
                ),
                EMode::Trim => {
