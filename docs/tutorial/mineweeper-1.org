...
This is the first a [[../index.html][series]] of tutorials to show how to use linkspace from python.
...
- You've read the [[http://www.linkspace.dev/basics.html][basics]].
...
Every player has to be at the same keyboard taking turns.
...
- Use the query to react to packets matching it - either from the local db or as they become available.

For Bob, the linkspace library is an abstraction such that all the players share a common tree where he can read from, and write into.

For a user of linkspace, Bob's game is just another domain application.
It provides friendly interface reading/writing the bytes that the developer wants to be shared around.
...
He ends up with a packet to start the game with that looks as follows:
...
But how do we determine whose turn it is?
There is no host to orchestrate, and packets can arrive for each players in a different order. Somebody might cheat :O?
In linkspace ordering some packets is actually trivial.
Adding a link to another packet is proof that one was created before the other.
