...
- Optionally: set a path, add one or more links to other bytes/packets, sign your point.
- Save the point.
...
For Bob building linkspace application, it offers him the abstraction that all players can address a shared space of points.
