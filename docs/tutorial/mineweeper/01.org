...
The follow knowledge/skills are assumed:
...
- As a plus, its nice if you've shared in the misery of administrating a server with user accounts/registrations.
...
The app developer can stick to: bytes, linking to bytes, and getting bytes from the either by a pointer or a path the developer chose. 

This tutorial won't be about those users and how they setup their linkspace instance and group,
but it will contain a few tips on emulating a local version for testing. 
...
But first, the game in hotseat mode: 

It reads in the game setup such as the list of players.
...
With linkspace:
- his (administrative) workload drops dramatically compared to some hosted solutions
- the game will stay playable without him spending a penny,
...
- Gather up some bytes you want sent around in a packet.
...
- A turn is a signed packet with json data containing [x,y] for the cell to reveal.
- Turns are ordered by having the first link be a pointer to the previous turn.
  the first turn will link pointer back to the game_pkt
...
#+begin_src python
  # we have to fix the create value, otherwise others can tweak it in their benefit. 
