...
For Bob, the linkspace library is an abstraction such that all the players share a common space where he can read from, and write into.
...
#+begin_src bash 
  ...
  lk link mineweeper:[#:test] --data ./setup.json | tee setup.pkt | lk printf
