...
Here we look for the domain 'my' , the group [#:pub], with path starting at /hello and with one additional component. 
#+BEGIN_SRC bash :session cli0 :exports both :results output verbatim :cache no
...
Each line is evaluated. you can set a pkt or argv context.
