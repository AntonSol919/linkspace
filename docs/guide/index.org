...
You should be familiar with either the [[../../code.html][quick start]] or the [[../../why.html][why]].
...
The function 'u8' turns a decimal repr of a byte into the byte.
...
Note that bytes are joined after evaluating. In the example this results in the byte component ~'hello '~.
The delimiters ('\n' '\t' ':' '/') are interpreted depends on the context.
For instance, ~lk eval~ prints them 'as is' when seen outside of ~[...]~

In other words: ~eval "hello/:[u8:99]\n"~ == ~hello/:c\n~ ,
but within brackets they have another meaning:  ~eval "hello/:[hello/ \n :]"~ will error.
