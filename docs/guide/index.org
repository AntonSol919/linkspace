...
#+BEGIN_VERSE
This is a living document it is kept up to date on a best effort basis.
Sometimes the [[../cargo-doc/liblinkspace/index.html][liblinkspace]] docs is ahead of this guide.
#+END_VERSE

* Introduction

Linkspace is a way to structure interlinked data into packets, addressable with a query, in order to build multi-user applications.
[[../cargo-doc/liblinkspace/index.html][liblinkspace]] is the authoritative implementation.

You should have seen the [[../../basics.html][basics]].
...
It sidesteps or solves a number of difficulties inherent in organizing data as streams in multi-user applications.
Such a system is classified as a supernet when hashed entries include hashes to other entries[fn::
...
This has upsides[fn::
The readme contains technical properties this provides. However, there is one I lack a word for.
...
This split and other standard problems, are supported by a set of [[Conventions]].
...
The max space is shared between the links and data so beware that too much data and links wont fit into a single packet
...
Its up to the user (or some to be build management tool) to pick a method of data exchange.
...
In addition, it supports evaluation various expressions (potentially reading a context such as a linkspace instance).
...
ABE is used for things like [[Query]] or simply [[DEFAULT_FMT][printing]] a text representation of a packet.

ABE is not meant to be a proramming language!
Its primarily meant to read and write arbitrary bytes in some context and quickly beat them into shape.
Some things are limited by design.

Instead an application should define what the bytes mean, and how to display them to the end user. 
If the goal is to display, you should read and write in UTF-8.
...
Understanding the ABE in depth is not critical.
...
- starting with '?' is a basic reverse operation. [u8:97/?u] == 97. Its similar but less powerful then [[lk_encode]], and unlike that the ?F don't return surrounding '[]' brackets, and they will error on failure.
...
They are used by the local index ( [[lk_watch]] and [[lk_get]] ),
...
#+RESULTS[7090d2515ed323cb265812647ca4093b57fe2249]:
#+begin_example
:mode:tree-desc
:watch:default
type:1:[b2:00000111]
...
#+begin_src python :session lkpy :exports both :results value verbatim
  q = lk_query()
  lk_query_push(q,"data_size","<",bytes([0,4])) # less then 4
  lk_query_push(q,"data_size","<",lk_eval("[u16:20]"))  # less then 20
  lk_query_push(q,"data_size","<",int(3).to_bytes(2))  # less then 3
  lk_query_print(q)
#+END_SRC
...
#+begin_src python :session lkpy :exports both :results value verbatim
  try:
    r = lk_query_push(q,"data_size",">",bytes([0,100])) #greater then 100
  except Exception as e :
    r = ("Thats not possible",e)
  r
...
#+RESULTS:
#+begin_example
('Thats not possible', RuntimeError("Error adding rule 'data_size'\n\nCaused by:\n    0: data_size:>:[u16:100]\n    1: incompatible Greater 100"))
#+end_example
...
You can nest calling ~lk_watch~ and ~lk_get~.
...
Every tag ending with '~^~' e.g. ~root_00^~ is a authority public key.
An authority has the right to vote for all sub claims. For example the claim ~lns:[#:pub]:/claim/test/example/john/home/~
...
You can hardcode a public key into the app, and also publish a group exchange service.
There are two primary ways to centralize things.
...
It shows what is required to write an simple app using linkspace.
...
#+begin_py
This is currently a straight up copy of the bash script.
