...
Depending on your use case, you can limit yourself to just those functions and write the rest of your network from scratch.
The rest are tools to make developing increasingly complex systems much easier. 
...
- [[#runtime][Runtime]] - A runtime around a multi-reader single-writer database for saving and querying.
...
The [[https://github.com/AntonSol919/linkspace/releases][download]] contains the CLI and python library and the examples used in this document.
...
The linkspace API is the Packet struct and a small set of functions.
...
- [[Runtime]] - A database to save packets and user-driven event loop to query for old and new packets.
  - [[lk_open]]
  - [[lk_save]]
  - [[lk_get]]
  - [[lk_get_all]]
...
| Spacename_{var<240} | Sequence of bytes. e.g. '/dir1/dir2/thing'         |
...
Whenever a cli commands deal with (domain, group, spacename) tuples, they are set by the first argument: ~DOMAIN:GROUP:SPACENAME~.
...
Its core structure is a stringly representation of delimited bytes ( of the type: ~[ ( [u8], delimiter ) ]~ ).
...
This adds functions such as ~hash~, ~group~, ~spacename~ etc.
...
The instance is thread local. Each thread or process requires calling [[lk_open]].
...
This includes calls to ~lk_get*~ and ~lk_watch~.  

Update the transaction and process all the new packets ([[lk_process]],[[lk_process_while]]).
