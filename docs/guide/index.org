...
The rule of thumb is that if the last element in an expression results in plain bytes, the entire list is concatenated including separators _as is_. 
...
The stamp and ubits are set by the exchange process. e.g. [[#anyhost][anyhost]]
