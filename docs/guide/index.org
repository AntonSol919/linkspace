...
#+BEGIN_VERSE
This is a living document, is not yet finished, things are broken, and is kept up to date on a best effort basis.
...
#+BEGIN_SRC bash :session cli :exports both :results output verbatim
lk --version
#+END_SRC
...
**** [[../cargo-doc/liblinkspace/point/fn.lk_datapoint.html][Rust]]
**** Bash
...
#+BEGIN_SRC bash :session cli :results output verbatim :cache yes
...
Linkspace has no concept of encoding. Fields have a fixed length or prefix their exact length.
...
They are used by the local index ( [[lk_view]] and [[lk_read]] ),
...
#+BEGIN_SRC bash :exports both :session cli :results output verbatim :cache yes
...
#+BEGIN_VERSE 
Note that many cli commands (e.g. ~print-statemnt~, ~view~ ) take as the first argument a ~domain:group:path:[?depth]~
If no depth is set the path_len is constraint by default.
Except for ~view-tree~ which sets the depth to unconstraint by default
#+END_VERSE 
...
| until_{8}    | Pkt set to lose meaning after Stamp |
| ubits0_{4}   | user defined u32                    |
| ubits1_{4}   | user defined u32                    |
| ubits2_{4}   | user defined u32                    |
| ubits3_{4}   | user defined u32                    |
...
You can nest calling ~lk_view~ and ~lk_read~.
...
* Conventions
Conventions are functions built from the other linkspace functions that encode a commonly used convention.
...
** lk_status
{x:status}:{#:0}:/DOMAIN/GROUP
For example {x:status}:{#:0}:/pull/{#:pub}/PULL_HASH"
