...
This is a technical document describing linkspace from first principle.
You should be familiar with the [[../../index.html#basics][basics]].
If you're interested in how linkspace solves your problems checkout the [[../tutorial/index.html][tutorials]].
...
If entries include hashes to other entries I consider it a supernet[fn::
Blockchains are a subset of supernets.
...
This has upsides[fn::
The homepage contains a list of various properties. However, there is one I lack a word for.
...
That's not to imply orchestration becomes simple, or the abstraction does not leak. I fear those are facts of life.
] and downsides.
...
- [[Point]] - Bytes that are named and reference other points by hash. Together with its hash and netheader form a packet.
...
Common challenges are addressed by a set of [[Conventions]].
...
The [[https://antonsol919.github.io/linkspace/index.html#download][zip]] contains the CLI and python library and the examples used in this document.
...
[fn::To overcome this, create a new linkpoint, either as a collecting or with a special tag that points to a next linkpoint] .
...
There are various ways to add data. The two common are ~--data stdin~ and ~--data file:./somefile~ .

#+BEGIN_SRC bash :session cli0 :exports both :results output verbatim
  echo hello | lk link my_domain --data stdin | lk printf
#+END_SRC
...
#+begin_src python :session lkpy :exports both :results output verbatim
  ...
  (p1, packet_bytes)= lk_read(packet_bytes,validate=True,allow_private=False)
  (p2, packet_bytes)= lk_read(packet_bytes,validate=True,allow_private=False)
  (p3, packet_bytes)= lk_read(packet_bytes,validate=True,allow_private=False)
...
#+BEGIN_SRC bash :session cli0 :exports both :results output verbatim :cache no
  echo datapoint:
  printf hello | lk data | tee /tmp/pkts | xxd 
  echo linkpoint:
  printf hello | lk link my_domain:[#:pub]:/hello/world -- link1:[#:0] link2:[#:test] | tee -a /tmp/pkts | xxd 
  echo keypoint:
  printf hello | lk keypoint --password "my secret" my_domain:[#:pub]:/hello/world -- link1:[#:0] link2:[#:test] | tee -a /tmp/pkts | xxd 
  cat /tmp/pkts | lk printf [hash:str]
#+END_SRC
#+end_bash
...
Knowing ABE is not a requirement. Depending on the context there are other ways.
E.g. In python you can stick to using bytes objects, where possible the cli accepts a base64 string, etc. 
Its a language of convenience. 
With basic knowledge of its purpose, expression substitution (~[..]~), and the ~':'~, ~'/'~ delimiters the reader can choose to
skip to the next chapter [[Query]] and return here for reference.
#+end_VERSE
...
#+BEGIN_VERSE
Available in the [[https://antonsol919.github.io/linkspace/index.html#download][pkg]] or in repository/examples
#+END_VERSE
...
#+begin_py
#+INCLUDE: "../../examples/linkmail/linkmail.py" src python
#+end_py
...
#+BEGIN_VERSE
Available in the [[https://antonsol919.github.io/linkspace/index.html#download][pkg]] or in repository/examples/app
#+END_VERSE
