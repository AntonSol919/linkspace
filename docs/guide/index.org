...
An exchange process (such as in the [[../tutorial/index.html][tutorial]]) watches these packets and attempts to fulfill it.
The exchange is only responsible for pull requests received when it is running.
The exchange can drop requests when you reuse the 'qid'.
