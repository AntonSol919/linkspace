...
An exchange process (such as those in the tutorial [[../tutorial/index.html][tutorials]]) watches these packets and attempts to fulfill it.
...
First off, if you desire to run SQL queries it is not too difficult to stream packets into a SQL table with `lk printf` or some custom code, and query them.
