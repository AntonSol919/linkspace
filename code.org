...
Tags can be anything. If less then 16 bytes are supplied it is left-padded with 0's.
...
A tool like `lk collect` has a few additional option for building points linking to other points.
...
To build an application pick a domain name (like you would an ip port - its also 16 bytes 0 padded).
...
A typical linkspace applications shouldn't have to think about sockets.
...
#+BEGIN_VERSE
The key take away here is - everything talks (streams of) packets. Avoid building custom serialization e.g. `struct MyStruct {customheader , packet}`.
